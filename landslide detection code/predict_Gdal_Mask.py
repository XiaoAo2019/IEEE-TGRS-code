...
from src import unet_mobilenet_v3_large
...
class GRID:
    ...
    def write_image(self, filename, img_proj, img_geotrans, img_data):
        ...
        image = driver.Create(filename, img_width, img_height, 1, datatype)
...
def detect_image(image):
    ...
    model = unet_mobilenet_v3_large(num_classes=classes+1)
...
def main():
    ...
    for i in range(0, len(count)):
        ...
        name = os.path.splitext(count[i])[0]
        # r_image.save(path_out+'{}.png'.format(str(i)))
        run.write_image(path_out + '{}.tif'.format(name), proj, geotrans, r_image)
