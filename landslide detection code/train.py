...
def create_model(num_classes, pretrain=True):
    model = MFFLnet(num_classes=num_classes, pretrain_backbone=True)
    if pretrain:
        model_path = 'ep250-loss0.004-val_loss0.005.pth'                                                                # pretrain weight
...
def main(args):
    ...
    model = create_model(num_classes=num_classes, pretrain=False)                                                       # create model
    model.to(device)
...
def parse_args():
    ...
    parser.add_argument("--data-path", default=r"./", help="VOCdevkit root")                                            # data path
